...
// Add states for branches and selectedBranch
code = code.replace(
  "const [storeProfile, setStoreProfile] = useState<any>(null);",
  "const [storeProfile, setStoreProfile] = useState<any>(null);\n  const [branches, setBranches] = useState<any[]>([]);\n  const [selectedBranch, setSelectedBranch] = useState<string>('all');\n  const [allTransactions, setAllTransactions] = useState<any[]>([]);"
);

// Fetch branches and store all transactions
const fetchLogic = `
...
// Add the branch filter dropdown to the UI header
code = code.replace(
  /<button className="flex items-center gap-2 text-sm font-bold text-slate-600 bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm hover:bg-slate-50">/,
  `{branches.length > 0 && (
              <select 
                value={selectedBranch} 
                onChange={e => setSelectedBranch(e.target.value)}
                className="text-sm font-bold text-indigo-700 bg-indigo-50 px-4 py-2 rounded-xl border border-indigo-100 shadow-sm hover:bg-indigo-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 mr-2"
              >
