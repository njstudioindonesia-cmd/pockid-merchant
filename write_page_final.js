...
import { 
  ...
  Wallet, Users, ShoppingBag, Settings, LogOut, Loader2, Bell
} from 'lucide-react';
          ...
          <nav className="space-y-1">
            {MENU_ITEMS.map((item) => (
              <Link 
                key={item.id}
                href={item.route}
                className={"flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-200 " + (activeTab === item.id ? 'bg-indigo-600/10 text-indigo-400 font-bold border border-indigo-500/20 shadow-inner' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50 font-medium')}
              >
                <item.icon className={"w-5 h-5 " + (activeTab === item.id ? 'text-indigo-400' : 'opacity-70')} />
                {item.label}
              </Link>
            ))}
          </nav>
